...
export function ServiceForm({ service, siteId }: { service: any; siteId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm p-6 space-y-5 max-w-2xl">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className={labelCls}>Title (English)</label><input {...register('title_en')} className={inputCls} />{errors.title_en && <p className="text-red-500 text-xs mt-1">{errors.title_en.message}</p>}</div>
      </div>
      <div><label className={labelCls}>URL (slug)</label><input {...register('slug')} placeholder="ej: implantes-dentales" className={inputCls} />{errors.slug && <p className="text-red-500 text-xs mt-1">{errors.slug.message}</p>}</div>
      <div className="grid grid-cols-2 gap-4">
        <div><label className={labelCls}>Descripción (Español)</label><textarea {...register('description_es')} rows={3} className={inputCls} /></div>
        <div><label className={labelCls}>Description (English)</label><textarea {...register('description_en')} rows={3} className={inputCls} /></div>
      </div>
      <div><label className={labelCls}>Imagen principal</label><ImageUploader folder="services" currentUrl={imageUrl} onUpload={(url) => setImageUrl(url)} /></div>
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={saving} className="bg-[#051c33] text-white px-5 py-2 rounded-lg text-sm hover:bg-[#051c33]/90 disabled:opacity-50 transition-colors">{saving ? 'Guardando...' : 'Guardar'}</button>
        <button type="button" onClick={() => router.push('/admin/servicios')} className="border border-[#8b9fb3]/40 text-[#051c33] px-5 py-2 rounded-lg text-sm hover:bg-[#eaeeef] transition-colors">Cancelar</button>
      </div>
    </form>
  )
}
